...
import com.speedtong.sdk.core.videomeeting.ECVideoMeetingSwitch;
 ...
 * 
 * <p>
 * Title:VideoConferenceChattingUI.java
 * </p>
 * <p>
 * Description: Set the main video, if the main video new set of main video
 * server ID and current ID the same, will cancel the compulsory primary video,
 * becomes automatic switching mode. So you can also use this method to achieve
 * the "abolition of compulsory primary video" function.
 * </p>
 * <p>
 ...
 * @version 3.5
 */
public class VideoConferenceChattingUI extends VideoconferenceBaseActivity
	...
	/**
	 * Broadcast action To monitor the alarm clock, every 30s for a local
	...
	/**
	 * To monitor the alarm clock, every 40s for download picture of Video
	 * Conference
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		registerReceiver(new String[] { ACTION_CCP_REVIEW_LOCAL_PORPRTAIT,
				ACTION_CCP_REVIEW_PORPRTAIT_REMOTE });
	...
	private void initialize(Bundle savedInstanceState) {
		...
		if (modeType == MODE_VIDEO_C_INITIATED_INTERCOM) {
			// ??
			// getDeviceHelper().startVideoConference(CCPConfig.App_ID,
			// roomName, 5, null,
			// null,is_auto_close,voiceMode,autoDelete==1?true:false,true);
		} else if (modeType == MODE_VIDEO_C_INVITATION) {

			// Initiate a join Viode Conference request, waiting for SDK to
			// return.
	...
	public synchronized Integer getCCPVideoConUIKey() {
		...
		ECLog4Util.v(SDKCoreHelper.TAG,
				"remove CCP Key out : " + UIKey.toString());
	...
	public synchronized void putCCPVideoConUIKey(Integer key) {
		...
		ECLog4Util.v(SDKCoreHelper.TAG, "put CCP Key into : " + key
				+ " ,UIKey " + UIKey.toString());
	}
	...
	public void putVideoUIMemberCache(String who, Integer key) {
		synchronized (mVideoMemberUI) {
			ECLog4Util.v(SDKCoreHelper.TAG, "put VideoUIMember who : " + who
					+ " , key " + key);
	...
	public boolean isVideoUIMemberCacheEmpty() {
		synchronized (mVideoMemberUI) {
			ECLog4Util.v(SDKCoreHelper.TAG, "VideoUIMember size : "
					+ mVideoMemberUI.size());
	...
	public Integer removeVideoUIMemberFormCache(String who) {
		synchronized (mVideoMemberUI) {
			...
			ECLog4Util.v(SDKCoreHelper.TAG, "remove VideoUIMember who : " + who
					+ " , key " + key);
			return key;
		}
	}
	...
	public Integer queryVideoUIMemberFormCache(String who) {
		synchronized (mVideoMemberUI) {
			if (!mVideoMemberUI.containsKey(who)
				...
				ECLog4Util.v(SDKCoreHelper.TAG,
						"Sorry , current VideoUI Member Cache not have " + who);
				return null;
			}
			Integer key = null;
			...
			ECLog4Util.v(SDKCoreHelper.TAG, "query VideoUIMember who : " + who
					+ " key is  " + key);
			return key;
		}
	}
	...
	public synchronized void initUIKey() {
		...
		ECLog4Util.v(SDKCoreHelper.TAG, "initUIKey ,UIKey " + UIKey.toString());
	}

	public String PORTRAIT_PATH = CCPUtil.getExternalStorePath() + "/"
			+ CCPUtil.DEMO_ROOT_STORE + "/.videoPortrait";
	...
	private void cancelAlarmTime(String action, int requestCode) {
		...
		PendingIntent pendingIntent = PendingIntent.getBroadcast(this,
				requestCode, new Intent(action),
				PendingIntent.FLAG_CANCEL_CURRENT);
		mAlarmManager.cancel(pendingIntent);
	}

	private void setAlarmTime(long triggerAtTime, int requestCode,
			long interval, String action) {
		isSynchronousData = true;
		PendingIntent pendingIntent = PendingIntent.getBroadcast(this,
				requestCode, new Intent(action),
				PendingIntent.FLAG_CANCEL_CURRENT);
		mAlarmManager.setRepeating(AlarmManager.RTC_WAKEUP, triggerAtTime,
	 ...
	 * 
	 * <p>
	 * Title: getVideoRemotePorprtaitDrawable
	 * </p>
	 * <p>
	 * Description:
	 * </p>
	...
	private BitmapDrawable getVideoRemotePorprtaitDrawable(
			ECVideoPartnerPortrait portrait) throws IOException {
		return CCPUtil.getImageDrawable(getPortraitFilePath(portrait.getVoip()
				+ "." + VoiceUtil.getExtensionName(portrait.getFileName())));
	}
	...
	private FrameLayout addFeatureGuide() {
		...
		videoMainView = getLayoutInflater().inflate(R.layout.video_conference,
				null);
	...
	private void setSycnAlarmTimeClock() {
		if (!isSynchronousData && isVideoChatting) {
			...
			setAlarmTime(System.currentTimeMillis() + 5 * 1000, 30 * 1000,
					ACTION_CCP_REVIEW_LOCAL_PORPRTAIT);
			setAlarmTime(System.currentTimeMillis() + 10 * 1000, 1, 40 * 1000,
					ACTION_CCP_REVIEW_PORPRTAIT_REMOTE);
		}
	}
	...
	public void exitOrDismissVideoConference(boolean dismiss) {
		...
		if (dismiss && isVideoConCreate) {
			...
			// getDeviceHelper().dismissVideoConference(CCPConfig.App_ID,
			// mVideoConferenceId); ??
		} else {
	...
	public void onClick(View v) {
		...
		case R.id.camera_control:
			...
			if (cameraCurrentlyLocked == android.hardware.Camera.CameraInfo.CAMERA_FACING_FRONT) {
				...
				Toast.makeText(VideoConferenceChattingUI.this,
	...
	public void onVideoUIItemClick(int CCPUIKey) {
		...
		CCPAlertDialog ccpAlertDialog = new CCPAlertDialog(
				VideoConferenceChattingUI.this, title, ccpAlertResArray, 0,
	...
	public void onItemClick(ListView parent, View view, int position,
		...
		case R.string.str_video_manager_self_main_screen:
			...
			if (!TextUtils.isEmpty(voipSwitch)) {
				showConnectionProgress(getString(R.string.str_dialog_message_default));
				// getDeviceHelper().switchRealScreenToVoip(CCPConfig.App_ID,
				// mVideoConferenceId, voipSwitch); ??
		...
		case R.string.str_video_manager_remove:
			...
			if (!TextUtils.isEmpty(voipRemove)) {
				showConnectionProgress(getString(R.string.str_dialog_message_default));
				// getDeviceHelper().removeMemberFromVideoConference(CCPConfig.App_ID,
	...
	private void doVideoConferenceDisconnect() {
		...
		CCPAlertDialog ccpAlertDialog = new CCPAlertDialog(
				VideoConferenceChattingUI.this, videoTips, null, videoExit,
				R.string.dialog_cancle_btn);
		ccpAlertDialog.setOnItemClickListener(this);
		ccpAlertDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				setSycnAlarmTimeClock();
				ECLog4Util.d(SDKCoreHelper.TAG, "CCPAlertDialog dismiss.");
			}
		});
		ccpAlertDialog.create();
		ccpAlertDialog.show();
	 ...
	 * 
	 * <p>
	 * Title: queryVideoMembersPorprtait
	 * </p>
	 * <p>
	 * Description: Query image on the local SDCARD According to the VOIP , if
	 * the local does not exist, then get the download address from a network
	 * </p>
	 * 
	...
	private void queryVideoMembersPorprtait(Integer ccpVideoConUIKey, String who)
		...
		if (videoFile != null && videoFile.exists()) {
			...
			BitmapDrawable imageDrawable = CCPUtil.getImageDrawable(videoFile
					.getAbsolutePath());
			mVideoConUI.setImageViewDrawable(ccpVideoConUIKey, imageDrawable);
		} else {
			mVideoConUI.setImageViewDrawableLoading(ccpVideoConUIKey, "加载中...",
					null);

			// Obtain the members the head
			// The interface will return all the video conference member avatar
			// download address
	 ...
	 * @param who
	 * 
	 * @see com.speedtong.example.meeting.ui.videomeeting.CCPVideoConUI#setOnVideoUIItemClickListener(OnVideoUIItemClickListener
	 *      l)
	 */
	private void setVideoUITextOperable(Integer ccpVideoConUIKey, String who) {

		if (isDisplayAllMembers) {
			mVideoConUI.setVideoUIText(ccpVideoConUIKey,
					CCPUtil.interceptStringOfIndex(who, 4), isVideoConCreate);
			return;
		}

		if (!isVideoConCreate || CCPConfig.VoIP_ID.equals(who)) {

			// If not the creator, so for all members of VideoUI are set to not
			// click operation
			// If self ,then do the same thing
			mVideoConUI.setVideoUIText(ccpVideoConUIKey,
					CCPUtil.interceptStringOfIndex(who, 4));
			return;
		}

		// Otherwise, display of operation and management of the icon,
		// and enable a listener of callback method that set by
		// CCPVideoConUI#setOnVideoUIItemClickListener(OnVideoUIItemClickListener
		// l)
		mVideoConUI.setVideoUIText(ccpVideoConUIKey,
				CCPUtil.interceptStringOfIndex(who, 4), true);
	...
	private void removeMemberFromVideoUI(Integer ccpVideoConUIKey) {
		if (ccpVideoConUIKey != null) {
			...
			ECLog4Util.v(SDKCoreHelper.TAG, "set VideoUI key "
					+ ccpVideoConUIKey + " null.");
		}
	}
	 ...
	 */
	private void checkPorprtaitCache(List<ECVideoPartnerPortrait> videoPortraits) {
		ArrayList<ECVideoPartnerPortrait> urlList = new ArrayList<ECVideoPartnerPortrait>();
		for (ECVideoPartnerPortrait portrait : videoPortraits) {
			try {
				...
				if (queryVideoUIMemberFormCache(who) == null
						|| CCPConfig.VoIP_ID.equals(who)) {
					// do nothing..
					// At present we deal only with the current video conference
					// member's Avatar
					// Ignore the other.
					ECLog4Util.d(SDKCoreHelper.TAG, "The portrait sender "
							+ who + " not in VideoConference "
							+ mVideoConferenceId);
					continue;
				}
				...
				// The member of video conference picture download address
				// stored in the cache.
				// Because self don't need pictures, so we ignore
				if (!CCPConfig.VoIP_ID.equals(who)) {
					ECVideoPartnerPortrait tempPortrait = mVideoPorprtaitCache
							.get(who);
					if (tempPortrait != null
							&& tempPortrait.getDateUpdate().equals(
									portrait.getDateUpdate())) {
						// If the avatar is updated only added to the download
						// queue
						isChange = false;
					}
				...
				if (queryVideoUIMemberFormCache(who) == CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW
					...
					// if the main screen member or self, then do ignore
					// download picture;
					// do nothing..
					continue;
				}
				...
				if (isChange) {
					// If the avatar is updated only added to the download queue
					File createPortraitFilePath = CreatePortraitFilePath(who,
							VoiceUtil.getExtensionName(portrait.getFileName()));
					if (createPortraitFilePath == null) {
						return;
					}
					portrait.setFileLocalPath(createPortraitFilePath
							.getAbsolutePath());
	 ...
	 * 
	 * <p>
	 * Title: initMembersOnVideoUI
	 * </p>
	 * <p>
	 * Description:Will refresh the list of members to VideoUI video do
	 * background thread .
	 * </p>
	 * 
	 * @param members
	 */
	private void initMembersOnVideoUI(List<ECVideoMeetingMember> members) {
		for (ECVideoMeetingMember member : members) {
			// Send an message to the message queue
			// Update VideoUI in the main thread
			Message obtainMessage = getBaseHandle().obtainMessage(
					WHAT_ON_VIDEO_REFRESH_VIDEOUI);
			Bundle b = new Bundle();

			if (member.getScreen() == 1) {
				putVideoUIMemberCache(member.getNumber(),
						CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW);

				b.putInt("ccpVideoConUIKey",
						CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW);
			} else {
				...
				if (isVideoUIMemberExist(member.getNumber())) {
					// Because this is just in the query members, if not start
					// query members,
	...
	protected void handleVideoConferenceState(int reason, String conferenceId) {
		...
		if (reason == 0) {
			...
			if (isVideoConCreate) {
				...
				// if self is create of Video Conference ,then put self CCPKey
				// in VideoUIMemberCache.
				// putVideoUIMemberCache(CCPConfig.VoIP_ID ,
				// CCPVideoConUI.LAYOUT_KEY_SUB_SURFACEVIEW);

				// If you are the creator, then will be locked for the main
				// screen to display video conference
				// Otherwise it will default to display the main screen
				// according to the volume of the sound
				// getDeviceHelper().switchRealScreenToVoip(CCPConfig.App_ID,
				// conferenceId, CCPConfig.VoIP_ID);??
			...
			ECLog4Util.d(SDKCoreHelper.TAG,
					" Sorry ,join Video Conference failed ...");
			// getDeviceHelper().exitVideoConference();
			ECDevice.getECMeetingManager().exitMeeting();
			Toast.makeText(getApplicationContext(),
					getString(R.string.str_join_video_c_failed, reason),
	...
	protected void handleReceiveVideoConferenceMsg(ECVideoMeetingMsg VideoMsg) {
		...
		synchronized (VideoConferenceChattingUI.class) {
			try {
				...
				if (VideoMsg instanceof ECVideoMeetingJoinMsg) {
				...
				} else if (VideoMsg instanceof ECVideoMeetingSwitch) {
					// Video conferencing switching the main screen (PUSH to all
					// staff room)
					ECVideoMeetingSwitch videoConferenceSwitch = (ECVideoMeetingSwitch) VideoMsg;
					...
					// String voip =
					// mVideoConUI.getVideoUIText(CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW);
					// 76
					...
					if (TextUtils.isEmpty(voipMainScreen)) {
						// If the main screen image is empty, then ignore the
						// sip message
						return;
					}
					...
					if (!isDisplayAllMembers) {
						// If you do not display all the members, when the
						// display
					...
					// Will be switched to a new main Screen of the VoIP into
					// the buffer queue
					putVideoUIMemberCache(who, /* ccpVideoConUIKey */
							CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW);

					// update the VideoUI of switch main screen.
					setVideoUITextOperable(
							CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW, who);

					// Set the frequency display area display frame members
					if (CCPConfig.VoIP_ID.equals(who)) {
						mVideoConUI
								.setVideoUIMainScreen(CCPVideoConUI.LAYOUT_KEY_SUB_SURFACEVIEW);
					} else {
						Integer ccpVideoConUIKeyWho = queryVideoUIMemberFormCache(who);
						if (ccpVideoConUIKeyWho != null) {
							mVideoConUI
									.setVideoUIMainScreen(ccpVideoConUIKeyWho);
						}
					}
					// Notice the status bar and who is switched into frequency
					updateVideoNoticeTipsUI(getString(
							R.string.video_tips_switch, who));
				}

			} catch (Exception e) {
				e.printStackTrace();
				ECLog4Util.i(SDKCoreHelper.TAG, "Sorry,update VideoUI error. "
						+ VideoMsg);
			}
		}
	}
	 ...
	 * 
	 * <p>
	 * Title: doSwitchScreenModel
	 * </p>
	 * <p>
	 * Description: If you do not display all the members, when the display a
	 * frequency when not displaying pictures
	 * </p>
	 * 
	 * @param who
	 * @return
	 ...
	 */
	@Deprecated
	private boolean doSwitchScreenModel(String voipMainScreen, String who)
			throws IOException {
		...
		if (CCPConfig.VoIP_ID.equals(who)) {
			...
			// You need to add a new sub VideoUI for removed from the main
			// screen placed member
			ccpVideoConUIKey = getCCPVideoConUIKey();
		} else {
		...
		if (ccpVideoConUIKey != null) {
			...
			if (CCPConfig.VoIP_ID.equals(voipMainScreen)) {
				...
				// Then put CCPKey back into the queue, and releases the screen
				// resources
				// remove the member of Video Conference form VideoUI
				removeMemberFromVideoUI(ccpVideoConUIKey);
				removeVideoUIMemberFormCache(voipMainScreen);
				// Will be switched to a new main Screen of the VoIP into the
				// buffer queue
				// putVideoUIMemberCache(who,
				// /*ccpVideoConUIKey*/CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW);
			} else {
	...
	// (List<ECVideoPartnerPortrait>) iTask.getTaskParameters("videoPortraits");
	// checkPorprtaitCache(videoPortraits);
	//
	// } else if (key == KEY_TASK_INIT_VIDEOUI_MEMBERS) {
	//
	// // init members on VideoUI
	// List<ECVideoMeetingMember> members = (List<ECVideoMeetingMember>)
	// iTask.getTaskParameters("members");
	// initMembersOnVideoUI(members);
	// } else if (key == TaskKey.TASK_KEY_VIDEO_FRAME) {
	//
	// VideoSnapshot snapshot =
	// ECDevice.getECVoipSetManager().getLocalVideoSnapshot();
	//
	// if(snapshot != null) {
	//
	...
	// setAlarmTime(System.currentTimeMillis() + 30*1000, 30*1000,
	// ACTION_CCP_REVIEW_LOCAL_PORPRTAIT);
	// }
	// @Override
	...
	//
	// @Override
	// protected void handleVideoConferenceRemoveMember(int reason, String
	// member) {
	...
	//
	// Integer integer =
	// queryVideoUIMemberFormCache(portrait.getVoip())/*mVideoMemberUI.get(portrait.getVoip())*/;
	// if(integer == null ) {
	// return;
	// }
	// if(mVideoConUI != null /*&& integer !=
	// CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW*/) {
	//
	// try {
	// setVideoUITextOperable(integer, portrait.getVoip());
	// mVideoConUI.setImageViewDrawable(integer,
	// getVideoRemotePorprtaitDrawable(portrait));
	// } catch (IOException e) {
	// e.printStackTrace();
	// }
	// }
	...
	// if(snapshot != null) {
	// if(CCPUtil.isExistExternalStore()) {
	// String fileName = CreatePortraitFilePath(PREFIX_LOCAL_VIDEO +
	// CCPConfig.VoIP_ID, "png").getAbsolutePath();
	...
	protected void handleNotifyMessage(Message msg) {
		...
		if (what == WHAT_ON_VIDEO_NOTIFY_TIPS) {
			...
			if (msg.obj instanceof Bundle) {
					...
					try {
						if (ccpVideoConUIKey >= 3) {
							...
							if (videoFile != null && videoFile.exists()) {
								...
								BitmapDrawable imageDrawable = CCPUtil
										.getImageDrawable(videoFile
												.getAbsolutePath());
								mVideoConUI.setImageViewDrawable(
										ccpVideoConUIKey, imageDrawable);

							} else {

								mVideoConUI.setImageViewDrawableLoading(
										ccpVideoConUIKey, "加载中...", null);
							}
						}

						if (members.equals(mVideoMainScreenVoIP)
								&& ccpVideoConUIKey != CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW) {
				...
				if (!TextUtils.isEmpty(mVideoMainScreenVoIP)
						&& mVideoMainScreenVoIP.equals(members)) {
					setVideoUITextOperable(
							CCPVideoConUI.LAYOUT_KEY_MAIN_SURFACEVIEW, members);
				}
			}
		}

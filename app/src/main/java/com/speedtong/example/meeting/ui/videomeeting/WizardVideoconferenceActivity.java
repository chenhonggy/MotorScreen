 ...
 * 
 * <p>
 * Title: WizardVideoConfActivity.java
 * </p>
 * <p>
 * Description: Video conference entrance interface. Introduction video
 * conferencing and application scenarios
 * </p>
 * <p>
...
public class WizardVideoconferenceActivity extends VideoconferenceBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		videoType = getIntent().getIntExtra(
				VideoconferenceConversation.CONFERENCE_TYPE, 0);
		if (videoType == 1) {
			doActoin();
			finish();
			return;
		...
		boolean first = CcpPreferences.getSharedPreferences().getBoolean(
				CCPPreferenceSettings.SETTINGS_VIDEO_CONF_FIRST.getId(),
				(Boolean) CCPPreferenceSettings.SETTINGS_VIDEO_CONF_FIRST
						.getDefaultValue());

		if (first) {
			try {
				CcpPreferences.savePreference(
						CCPPreferenceSettings.SETTINGS_VIDEO_CONF_FIRST, true,
						true);
	...
	private void doActoin() {
		Intent intent = new Intent(WizardVideoconferenceActivity.this,

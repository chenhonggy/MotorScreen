...
import com.speedtong.example.meeting.common.utils.CCPDrawableUtils;
...
import com.speedtong.example.meeting.view.CCPButton;
 ...
 * 
 * <p>
 * Title: CreateVideoConference.java
 * </p>
 * <p>
 * Description:
 * </p>
...
public class CreateVideoConference extends VideoconferenceBaseActivity
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mCcpTitleViewBase
				.setCCPTitleViewText(getString(R.string.str_button_new_video_conference));
		...
		case R.id.create_video_c_submit:
			...
			if (!cb_autojoin.isChecked()) {
				if (mVideoConfType == 0) {
					...
					builder.setAppId(CCPConfig.App_ID)
							.setMeetingName(
					...
					ECDevice.getECMeetingManager().createMultMeetingByType(
							params, ECMultMeetingType.ECMeetingVideo,
			...
			intent.putExtra(VideoconferenceConversation.CONFERENCE_CREATOR,
					CCPConfig.VoIP_ID);
			intent.putExtra(ChatroomName.CHATROOM_NAME, mVideoCEditText
					.getText().toString());
			intent.putExtra(ChatroomName.IS_AUTO_CLOSE,
